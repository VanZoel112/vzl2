...
# Music directory
MUSIC_DIR = Path("downloads/musik_pytube")
...
async def vzoel_init(client, emoji_handler):
    ...
    print(f"{signature} PyTube Musik Plugin loaded - Stable music system ready")
...
async def search_youtube_pytube(query, limit=5):
...
async def download_audio_pytube(url, output_dir):
...
async def play_music_pytube(event):
...
{get_emoji('utama')} 𝗩𝗭𝗢𝗘𝗟 𝗔𝗦𝗦𝗜𝗦𝗧𝗔𝗡𝗧 Music Search"""
        ...
        # Search for music
        results = await search_youtube_pytube(query, limit=3)
...
async def download_music_pytube(event):
...
{get_emoji('utama')} 𝗩𝗭𝗢𝗘𝗟 𝗔𝗦𝗦𝗜𝗦𝗧𝗔𝗡𝗧 Downloader"""
            ...
            results = await search_youtube_pytube(query, limit=1)
            if not results:
...
{get_emoji('utama')} 𝗩𝗭𝗢𝗘𝗟 𝗔𝗦𝗦𝗜𝗦𝗧𝗔𝗡𝗧 Downloader"""
        ...
        # Download the audio
        file_path, status = await download_audio_pytube(url, MUSIC_DIR)
...
async def search_music_pytube(event):
    ...
    if event.is_private or event.sender_id == (await event.client.get_me()).id:
        ...
        results = await search_youtube_pytube(query, limit=5)
...
async def music_info_pytube(event):
