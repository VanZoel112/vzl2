...
{vzoel_emoji.get_emoji('telegram')} **Chat:** {chat.title}
{vzoel_emoji.get_emoji('utama')} **PyTgCalls:** {pytgcalls_status}
{vzoel_emoji.get_emoji('aktif')} **Connection:** {vc_connection}
{vzoel_emoji.get_emoji('proses')} **Audio:** {audio_status}

{vzoel_emoji.get_emoji('centang')} **Available Commands:**
...
{vzoel_emoji.get_emoji('loading')} **Installation Required:**

{vzoel_emoji.get_emoji('utama')} **Step 1:** Install PyTgCalls
```bash
pip install py-tgcalls -U
```

{vzoel_emoji.get_emoji('centang')} **Step 2:** Install FFmpeg (if needed)
```bash
# Ubuntu/Debian
apt install ffmpeg
...
{vzoel_emoji.get_emoji('aktif')} **Step 3:** Restart VzoelFox Assistant
```bash
.restart
```

{vzoel_emoji.get_emoji('telegram')} **Features After Installation:**
• Join voice chats in groups
• Mute/unmute microphone
• Professional voice chat management
• Audio streaming capabilities

{vzoel_emoji.get_emoji('petir')} **Note:** Voice chat requires user account (not bot)
