...
{get_emoji('petir')} Generated by Vzoel Fox's Card System"""
...
async def profile_info_handler(event):
    ...
    if event.is_private or event.sender_id == (await event.client.get_me()).id:
        ...
        profile_info = f"""{signature} Vzoel Fox's Profile Card Generator
...
• Watermark Vzoel Fox's branding
...
{get_emoji('biru')} Powered by Vzoel Fox's Technology
